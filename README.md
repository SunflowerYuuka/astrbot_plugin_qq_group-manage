...
- **多模型轮询兜底**：支持在 WebUI 中配置多个备用大模型。当主模型因网络波动或无余额报错时，自动无缝切换备用模型重试。
- **静态文本兜底**：当所有大模型均不可用时，自动发送预设的静态兜底文本，确保迎新、送辞、审核流程永不中断。
...
[新增] 多模型轮询兜底机制，支持配置备用大模型。

[新增] 静态文本兜底功能，防止大模型全部失效时机器人无响应。
